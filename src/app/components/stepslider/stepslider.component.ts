...
export class StepsliderComponent implements OnInit {
  ...
  initSteps() {
    this.steps = this.steps.sort(function(a,b) {return a-b}); // sort low to high
    ...
    for (var i = 0; i<this.steps.length;i++) {
      ...
      this.stepsLocations.push({location:l, percent:l / this.width * 100, value:this.steps[i]});
    }
  }
  ...
  slide(value:number, elemPerc:number, exactPx:number = null) {
    ...
    this.setValue(foundSeparator["value"]);
